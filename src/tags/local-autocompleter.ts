 ...
 */
export default class LocalAutocompleter {
  ...
  topK(prefix: string, k: number): Result[] {
    ...
    const prefixMatch = (name: string) =>
      strcmp(name.slice(0, prefix.length), prefix);
    this.scanResults(this.getResultAt.bind(this), prefixMatch, results);

    // Find in secondary order
    const namespaceMatch = (name: string) =>
      strcmp(nameInNamespace(name).slice(0, prefix.length), prefix);
    this.scanResults(
